...
const Chat = ({
  ...
  return (
    <li
      onClick={() => selectedUserClickHandler(userId)}
      key={chatId}
      className={`flex items-center p-4 relative ${activeChatClassName(
        userId,
        selectedUserId
      )} hover:bg-dark-60 cursor-pointer  rounded-[10px] mb-4`}
    >
      ...
      <div className="flex flex-1 flex:row lg:flex-col xl:flex-row">
        <div className="flex-1 shrink-0">
          ...
          <p
            className={`text-sm text-light-60 ${classNames({
              "font-bold":
                recentMessage.messageStatus === "SENT" &&
                recentMessage.sender.id !== user?.id,
            })}`}
          >
            {recentMessage.sender.id === user?.id
              ? `You: ${textOverflowFix(recentMessage.message, 20)}`
              : textOverflowFix(recentMessage.message, 20)}
          </p>
        </div>
        <div className="flex flex-col items-end lg:flex-row xl:flex-col">
          <p className="mt-2 mb-2 text-xs text-left text-light-40 lg:mb-0 lg:mr-2 xl:mr-0 xl:mb-2">{`${convertToLocaleTime(
