...
const MessageBox = ({
  ...
  return (
    <motion.li
      ref={messageRef}
      id={`message-${id}`}
      className={`flex rounded-[10px] transition-all ${classNames({
    ...
    >
      ...
      >
        ...
        >
          {parent && (
            <Link href={`/#message-${parent.id}`}>
              <div className="p-2 bg-dark-100 mb-1 rounded-[10px]">
                <p className="text-xs font-bold text-light-40">
                  {parent.sender.id === user?.id
                    ? "You"
                    : parent.sender.username}
                </p>
                <p className="text-xs text-light-40">
                  {textOverflowFix(parent.message, 153)}
                </p>
              </div>
            </Link>
          )}
