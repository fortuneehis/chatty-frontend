...
const withDrawer = <T extends { showDrawer: boolean}>(Component: ComponentType<T>)=> (matches: MediaQueryAllQueryable)=> (props: Omit<T, "match">): JSX.Element=> {
    ...
    const [mediaQueryMatch, setMediaQueryMatch] = useState(match)
        ...
        return ( 
            ...
            {mediaQueryMatch ? (
                <AnimatePresence>
                    {
                        props.showDrawer &&<Component {...props as T} match={mediaQueryMatch} /> 
                    }
                </AnimatePresence>
            ) : (
