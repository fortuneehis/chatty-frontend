    ...
    <div className="relative mt-12">
      <div className="bg-dark-60 rounded-[10px] flex px-4">
        ...
        <input
          ...
          value={value}
          className="w-full bg-transparent outline-none text-light-80 placeholder:text-light-60"
          placeholder="Search"
        />
      </div>
      {showResults && (
        <div className="top-full mt-2 block z-50 overflow-auto p-2 absolute left-0 w-full  max-h-[200px] rounded-[10px] bg-dark-60">
          {searchResults.length > 0 ? (
            searchResults?.map((user: any) => (
              <div
                key={user.id}
                onClick={() => resultClickEventHandler(user.id)}
                className={`p-4 cursor-pointer hover:bg-dark-80 ${classNames({
